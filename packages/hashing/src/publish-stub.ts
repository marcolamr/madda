/**
 * Default `config/hashing.ts` when published via `madda config:publish hashing`.
 * Driver defaults to `argon2id` (Laravel's published default is `bcrypt`).
 */
export const HASHING_CONFIG_PUBLISH_STUB = `import type { HashingConfig } from "@madda/hashing";
